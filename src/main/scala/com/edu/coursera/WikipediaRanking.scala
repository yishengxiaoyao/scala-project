...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.textFile(WikipediaData.filePath).map(x => {
    WikipediaData.parse(x)
  })
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    for (lang <- langs){
      result.append((lang,occurrencesOfLang(lang,rdd)))
    }
    result.sortBy(_._2).reverse.toList
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(article => langs.filter(lang=> article.mentionsLanguage(lang)).map(item=>(item,1)))
