...
import com.edu.stackoverflow.{Answer, HighScore, LangIndex, QID, Question}
...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.mapValues(ps => {
      (ps.head._1,answerHighScore(ps.map(_._2).toArray))
    }).values
  }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val medianScore: Int    = if (xs.length == ys.length) (xs.last + ys.last) / 2 else ys.head
